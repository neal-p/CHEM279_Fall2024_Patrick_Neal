...
def stretch_water(geom, H_to_move, n):

    bond_length = np.linalg.norm(geom[O_IDX] - geom[H_to_move])

    disp = bond_length * 0.3
    ...
    for idx, l in enumerate(raw_bond_lengths):
        ...
        if l < 0.7: # dont go below 0.7, its just goint to spike the energy
            bond_lengths.append(np.random.uniform(0.7, 1.0))

        elif l > 1.5:
            bond_lengths.append(np.random.uniform(1.1, 1.5))
    ...
    for bl in bond_lengths:
        ...
        for idx in to_move:
            vec = geom[idx] - geom[O_IDX]
            updated[idx] = geom[O_IDX] + vec * scale
...
def Sample(stem, n):
    ...
    info = parsers.read_mopac_freq_info(stem)
    info["temp"] = 298.15
...
if __name__ == "__main__":
    ...
    elements, coords = parsers.read_xyz(xyz)
    stem = ".".join(xyz.split(".")[:-1])
    n = 500

    wigner_structures = [coords] + Sample(stem, n)
    ...
    for geom in wigner_structures:
        ...
        for stretched_A in stretch_water(geom, H1_IDX, 50):
           all.append(stretched_A)

           for stretched_B in stretch_water(stretched_A, H2_IDX, 50):
               all.append(stretched_B)

    to_keep = int(min(max, len(all))) - 1

    # always keep original geometry
    all = [coords] + [all[i] for i in np.random.choice(list(range(len(all))), to_keep)]

    for idx, geom in enumerate(all):
        parsers.write_class_xyz(elements, geom, f"sampled_structrue_{idx}.xyz")

    parsers.write_multi_xyz(elements, all, f"all_geometries.xyz")
