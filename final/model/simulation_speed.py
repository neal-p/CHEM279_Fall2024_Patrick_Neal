...
def simulation(coords, num_steps, predictor):
    ...
    energy, forces = predictor.predict(coords)
    ...
    for step in range(num_steps):
        ...
        H1_O = coords[1] - coords[0]
        bd1 = np.linalg.norm(H1_O)
        H2_O = coords[2] - coords[0]
        bd2 = np.linalg.norm(H2_O)

        if bd1 > 2.0:
            correction = (bd1 - 2.0) * (H1_O / bd1)  # Adjust distance to 2.0
            coords[1] -= correction / 2  # Move H1 slightly
            coords[0] += correction / 2  # Move O slightly to conserve COM

        if bd2 > 2.0:
            correction = (bd2 - 2.0) * (H2_O / bd2)  # Adjust distance to 2.0
...
if __name__ == "__main__":
    ...
    start = time.time()
    simulation(coords, num_steps, predictor)
    end = time.time()
