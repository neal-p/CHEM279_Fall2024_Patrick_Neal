...
from parse import get_atomic_coordinates
...
def get_reduced_mass(file):
    ...
    if vib_section == 0:
        raise ValueError(f"Could not read reduced masses from {file}")
...
def ReadMolden(input):
    ...
    # Need a few extra things directly from the mopac output since they are not included
    # in the mopac -> molden conversion :(

    elements, coords = get_atomic_coordinates(f"{input}.out")
    natom = len(elements)
    rmass = get_reduced_mass(f"{input}.out")
    nmode = len(rmass)

    with open('%s.molden' % (input),'r') as molden:
